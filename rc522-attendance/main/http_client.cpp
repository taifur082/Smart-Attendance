...
esp_err_t http_client_send_scan_data(const scan_data_t* data)
{
    ...
    cJSON_AddStringToObject(json, "tag_type", "rc522");
    ...
    config.transport_type = HTTP_TRANSPORT_OVER_TCP;  /* plain HTTP, no TLS */
    ...
    for (int i = 0; i < HTTP_RETRY_COUNT; i++) {
        ...
        if (ret == ESP_OK) {
            ...
            if (status_code == 404) {
                /* Server received request but student not found; treat as delivered so we don't resend. */
                ESP_LOGW(TAG, "Student not found (404) for this UID");
                ret = ESP_OK;
                break;
            }
        ...
        if (i < HTTP_RETRY_COUNT - 1 && ret != ESP_OK) {
